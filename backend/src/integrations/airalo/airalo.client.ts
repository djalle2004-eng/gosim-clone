...
class AiraloClient {
  ...
  async authenticate(): Promise<string> {
    ...
    if (!providersConfig.airalo.clientId) {
        console.warn('AIRALO_CLIENT_ID missing, falling back to mock token (or failing).');
    ...
    try {
      ...
      throw new Error(`Airalo Auth Failed: ${error?.response?.data?.message || error.message}`);
    }
  }
  ...
  async getPackage(packageId: string) {
      if(providersConfig.airalo.useMock) return this.mockSinglePackage(packageId);
  ...
  async createOrder(packageId: string, quantity: number = 1) {
      if(providersConfig.airalo.useMock) return this.mockCreateOrder(packageId, quantity);
  ...
  private mockSinglePackage(id: string) {
      return { id, title: "Mocked Package", price: 15.00, data: "10 GB", validity: "30 days" };
  }
