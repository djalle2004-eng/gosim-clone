...
export class AiraloOrderService {
    ...
    async provisionOrder(orderId: string) {
        ...
        // Move to PROCESSING
        await db.order.update({ where: { id: orderId }, data: { status: OrderStatus.PROCESSING } });
        ...
        for (const item of order.orderItems) {
            ...
            for (let i = 0; i < item.quantity; i++) {
                try {
                    await this.provisionSingleItem(order.userId, item.id, item.plan.providerId, item.plan.dataAmount);
                } catch (error) {
                    allSuccessful = false;
                    console.error(`❌ Failed to provision Airalo item (Plan ${item.plan.name}):`, error);
                }
            }
        }

        if (allSuccessful) {
            await db.order.update({ where: { id: orderId }, data: { status: OrderStatus.COMPLETED } });
            console.log(`✅ [Provisioning] Order ${orderId} fulfilled successfully!`);
        } else {
            console.warn(`⚠️ [Provisioning] Order ${orderId} fulfilled partially or failed.`);
            // In a real app, send alert to Support manually here.
        }
    }

    private async provisionSingleItem(userId: string, orderItemId: string, airaloPackageId: string, dataTotalMB: number) {
        // 1. Send Create Order Request to Airalo
        const airaloDraftParams = await airaloClient.createOrder(airaloPackageId, 1);
        ...
        if (!sim || !sim.lpa) throw new Error("Airalo returned completed order without generic LPA string.");

        // 4. Fallback generate QRCode visually if Airalo's string URL misses
        const qrCodeBase64 = sim.qrcode_url ? sim.qrcode_url : await QRCode.toDataURL(sim.lpa);
     ...
     */
    private async pollOrderCompletion(airaloOrderId: string | number, maxAttempts = 24): Promise<any> {
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            ...
            if (orderRes.status === 'failed' || orderRes.status === 'canceled') {
                throw new Error(`Airalo processing strictly failed with status: ${orderRes.status}`);
            }
