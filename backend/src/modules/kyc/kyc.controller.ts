...
export const uploadDocument = async (req: Request, res: Response) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json({ message: 'Aucun fichier détecté. Veuillez attacher un document.' });
    }

    const { documentType } = req.body;
    if (!Object.values(DocumentType).includes(documentType)) {
      return res.status(400).json({ message: 'Type de document invalide (PASSPORT, NATIONAL_ID, DRIVERS_LICENSE).' });
    }
    ...
    const kycRecord = await createOrUpdateKyc(user.id, documentType as DocumentType, fullUrl);
