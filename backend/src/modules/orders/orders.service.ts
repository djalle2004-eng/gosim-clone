...
export const createOrder = async (userId: string, planId: string, quantity: number, currency: any, price: number) => {
...
export const getUserOrders = async (userId: string) => {
  return await prisma.order.findMany({
    where: { userId },
    include: {
      orderItems: { include: { plan: { include: { country: true } }, assignedESims: true } }
    },
...
export const refundOrder = async (orderId: string) => {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    include: { payments: true, orderItems: { include: { assignedESims: true } } }
  });

  if (!order || order.status !== 'PAID') throw new Error('Invalid order for refund');
  ...
  for (const esim of esims) {
    if (esim.dataUsed > 0) {
      throw new Error(`Refund aborted: ICCID ${esim.iccid} has already consumed ${esim.dataUsed}MB data`);
    }
  }

  const payment = order.payments.find(p => p.status === 'COMPLETED' && p.provider === 'STRIPE');
