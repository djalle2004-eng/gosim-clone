...
export const createOrder = async (req: Request, res: Response) => {
  ...
  sendResponse(
    req,
    res,
    { orderId: `ord_${Date.now()}`, status: 'PROCESSING', planId, quantity },
    null,
    201
  );
};
...
export const getEsimQrCode = async (req: Request, res: Response) => {
  sendResponse(req, res, {
    iccid: req.params.iccid,
    qrCodeUrl: 'https://api.gosim.dz/qrcodes/mock.png',
  });
};
...
export const getBalance = async (req: Request, res: Response) => {
  sendResponse(req, res, { balance: 500.0, currency: 'USD' });
};

export const topupBalance = async (req: Request, res: Response) => {
  sendResponse(req, res, {
    message: 'Invoice generated. Please pay to update balance.',
