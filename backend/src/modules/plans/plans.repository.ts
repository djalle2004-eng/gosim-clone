...
export const getPlans = async (where: Prisma.ESimPlanWhereInput, orderBy: any, skip: number, take: number) => {
  const [total, plans] = await Promise.all([
    ...
    prisma.eSimPlan.findMany({
      ...
      take,
      include: {
        country: { select: { nameEn: true, code: true, flag: true, region: true } },
