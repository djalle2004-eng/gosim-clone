...
export class InvoiceService {
  ...
  static async generateInvoice(orderId: string): Promise<Uint8Array> {
    ...
    // --- Header ---
    page.drawText('SoufSim', { x: 50, y, size: 24, font: boldFont, color: rgb(0.03, 0.45, 0.82) });
    page.drawText('INVOICE', { x: width - 150, y, size: 24, font: boldFont, color: rgb(0.2, 0.2, 0.2) });
    
    y -= 40;
    page.drawText(`Invoice Number: INV-${order.id.slice(0, 8).toUpperCase()}`, { x: width - 250, y, size: 10, font });
    y -= 15;
    page.drawText(`Date: ${order.createdAt.toISOString().split('T')[0]}`, { x: width - 250, y, size: 10, font });
    y -= 15;
    page.drawText(`Status: ${order.status}`, { x: width - 250, y, size: 10, font });

    // --- Billed To ---
    y -= 40;
    page.drawText('Billed To:', { x: 50, y, size: 12, font: boldFont });
    y -= 15;
    page.drawText(`${order.user.firstName || ''} ${order.user.lastName || ''}`, { x: 50, y, size: 10, font });
    y -= 15;
    page.drawText(order.user.email, { x: 50, y, size: 10, font });
    
    if (order.user.taxId) {
      y -= 15;
      page.drawText(`Tax ID / VAT: ${order.user.taxId}`, { x: 50, y, size: 10, font });
    }
    ...
    order.orderItems.forEach((item) => {
      ...
      page.drawText(`$${item.totalPrice.toFixed(2)}`, { x: 480, y, size: 10, font });
      y -= 20;
    });

    // --- Totals ---
    y -= 20;
    page.drawLine({ start: { x: 350, y: y + 10 }, end: { x: width - 50, y: y + 10 }, thickness: 1, color: rgb(0.8, 0.8, 0.8) });
    ...
    y -= 15;
    page.drawText(`Tax (${order.taxRate}%):`, { x: 380, y, size: 10, font: boldFont });
    page.drawText(`$${order.taxAmount.toFixed(2)}`, { x: 480, y, size: 10, font });

    y -= 20;
    page.drawText('Total Amount:', { x: 380, y, size: 12, font: boldFont });
    page.drawText(`$${order.totalAmount.toFixed(2)}`, { x: 480, y, size: 12, font: boldFont, color: rgb(0.03, 0.45, 0.82) });
    ...
    page.drawText('Scan to verify this invoice', { x: 50, y: 35, size: 8, font, color: rgb(0.5, 0.5, 0.5) });

    // --- Footer ---
    page.drawText('Thank you for choosing SoufSim!', { x: width / 2 - 80, y: 30, size: 10, font: boldFont });
