...
export default function CartReviewStep({ paymentMethod, setPaymentMethod, onNext, isProcessing }: CartReviewStepProps) {
  ...
  if (cart.items.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-slate-200">
        ...
        <p className="text-slate-500 mb-8">لم تقم بإضافة أي باقات إلى سلتك بعد.</p>
        <button onClick={() => window.location.href='/plans'} className="bg-cyan-500 text-slate-900 px-8 py-3 rounded-xl font-bold">
          تصفح الباقات
        </button>
      </div>
    );
    ...
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className="lg:w-2/3 space-y-6">
        <div className="bg-white rounded-3xl border border-slate-200 p-6">
          <h2 className="text-xl font-bold text-slate-800 mb-6 border-b pb-4">مراجعة السلة</h2>
              ...
              <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{item.isUnlimited ? 'بيانات غير محدودة' : `${item.dataAmount} GB`}</p>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl p-1">
                    <button onClick={() => cart.updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-600">-</button>
                    <span className="font-medium text-slate-800 w-4 text-center">{item.quantity}</span>
                    <button onClick={() => cart.updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-600">+</button>
                  </div>
                  <div className="text-right w-24">
                    <div className="font-bold text-slate-800">{item.priceDzd * item.quantity} د.ج</div>
                  </div>
                  <button onClick={() => cart.removeItem(item.id)} className="text-red-400 hover:text-red-500 p-2 bg-red-50 hover:bg-red-100 rounded-xl transition-colors">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 flex items-center gap-4">
          ...
          />
          <button onClick={handleApplyPromo} className="bg-slate-800 text-white px-6 py-3 rounded-xl font-medium hover:bg-slate-700">
            تطبيق
          </button>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6">
          <h2 className="text-xl font-bold text-slate-800 mb-6 border-b pb-4">اختر طريقة الدفع</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { id: 'cib', label: 'CIB', icon: <Landmark />, color: 'text-green-600', bg: 'bg-green-50 border-green-200' },
              { id: 'edahabia', label: 'الذهبية', icon: <Phone />, color: 'text-yellow-600', bg: 'bg-yellow-50 border-yellow-200' },
              { id: 'stripe', label: 'Stripe', icon: <CreditCard />, color: 'text-indigo-600', bg: 'bg-indigo-50 border-indigo-200' },
              { id: 'wallet', label: 'المحفظة', icon: <Wallet />, color: 'text-cyan-600', bg: 'bg-cyan-50 border-cyan-200' },
            ].map(method => (
              <button
                key={method.id}
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 sticky top-32">
          ...
          <div className="space-y-4 mb-6 text-slate-600">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-slate-800">{cart.getTotalDzd().toLocaleString()} د.ج</span>
            </div>
            {cart.discountRate > 0 && (
          ...
          <div className="border-t border-slate-200 pt-6 mb-8">
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-3xl font-black text-cyan-500">{cart.getFinalTotalDzd().toLocaleString()} د.ج</span>
            </div>
            <p className="text-sm text-slate-400 text-right">~ ${cart.getTotalUsd().toFixed(2)} USD</p>
          </div>
