...
export default function ConfirmationStep({ orderId }: ConfirmationStepProps) {
  ...
  const { data: order, isLoading } = useQuery({
    ...
    refetchInterval: (query) => query.state.data?.status === 'PAID' && query.state.data?.orderItems?.[0]?.assignedESims?.length > 0 ? false : 3000,
  });
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xl">
        <div className="bg-gradient-to-r from-emerald-400 to-cyan-500 p-8 text-center text-white">
          ...
          <p className="text-emerald-50">طلبك رقم #{order?.id.substring(0,8).toUpperCase()} قيد التنفيذ.</p>
        </div>
          ...
          {assignedEsim ? (
            <div className="flex flex-col md:flex-row gap-8 items-center bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                <img src={assignedEsim.qrCodeUrl} alt="eSIM QR Code" className="w-48 h-48 object-contain" />
              </div>
              <div className="flex-1 space-y-4">
                <div>
                  <h3 className="font-bold text-slate-800 text-xl flex items-center gap-2">
                    <QrCode className="text-cyan-500" /> امسح الكود لتفعيل الشريحة
                  </h3>
                  <p className="text-slate-500 mt-1">افتح كاميرا هاتفك وامسح الـ QR Code لتثبيت باقة البيانات فوراً.</p>
                </div>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-3 flex justify-between items-center">
                  ...
                  <button 
                    onClick={() => navigator.clipboard.writeText(assignedEsim.lpaString)}
                ...
                <p className="text-xs text-slate-400">يمكنك استخدام الكود أعلاه للتفعيل اليدوي إذا لم ينجح الـ QR.</p>
              </div>
            </div>
          ) : (
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 p-6 rounded-2xl mb-8 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm">يتم الآن تجهيز شريحتك مع مزود الخدمة. يرجى الانتظار، سيظهر كود التفعيل هنا تلقائياً خلال ثوانٍ...</p>
              </div>
            </div>
          )}

          <div className="grid md:grid-cols-3 gap-4">
            <Link to="/dashboard/esims" className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-xl font-bold transition-colors">
              عرض شرائحي (eSIMs)
            </Link>
