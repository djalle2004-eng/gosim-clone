      ...
      <main className="flex-grow pt-32 pb-20 container mx-auto px-4 md:px-6">
        ...
        <div className="flex justify-center items-center mb-12">
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div
              className={`w-16 h-1 rounded-full ${step >= 2 ? 'bg-cyan-500' : 'bg-slate-200'}`}
            />
            <StepIndicator current={step} target={2} label="الدفع" />
            <div
              className={`w-16 h-1 rounded-full ${step >= 3 ? 'bg-cyan-500' : 'bg-slate-200'}`}
            />
...
function StepIndicator({
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div
        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${active ? 'bg-cyan-500 text-slate-900 shadow-lg shadow-cyan-500/30' : 'bg-white border-2 border-slate-200 text-slate-400'}`}
      >
        {target}
      </div>
      <span
        className={`text-sm font-medium ${active ? 'text-slate-800' : 'text-slate-400'}`}
      >
        {label}
      </span>
    </div>
