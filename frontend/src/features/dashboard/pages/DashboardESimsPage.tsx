  ...
  const esims = orders.flatMap((o: any) =>
    o.orderItems.flatMap((item: any) =>
      item.assignedESims.map((e: any) => ({
        ...
        countryName:
          item.plan.country?.nameAr || item.plan.country?.nameEn || '',
        ...
        daysRemaining: e.expiresAt
          ? Math.max(
              0,
              Math.ceil(
                (new Date(e.expiresAt).getTime() - Date.now()) / 86400000
              )
            )
          : item.plan.validity,
        iccid: e.iccid,
      ...
      <div className="flex gap-2 mb-6 bg-white border border-slate-200 rounded-2xl p-1 w-fit">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            <span
              className={`mr-2 text-xs px-1.5 py-0.5 rounded-full ${tab === t.key ? 'bg-white/30 text-white' : 'bg-slate-100 text-slate-500'}`}
            >
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-slate-200/60 rounded-2xl p-12 text-center">
          ...
          <p className="text-slate-500 text-lg font-medium">
            لا توجد شرائح في هذه الفئة
          </p>
        </div>
      ) : (
        <div className="grid lg:grid-cols-2 gap-5">
          {filtered.map((esim: any, idx: number) => {
            const pct = esim.isUnlimited
              ? 100
              : Math.min(
                  100,
            ...
            return (
              ...
              >
                {/* Card Header */}
                <div
                  className={`p-5 ${esim.status === 'ACTIVE' ? 'bg-gradient-to-l from-emerald-50 to-white' : 'bg-slate-50'}`}
                >
                ...
                <div className="px-5 py-4">
                  ...
                  <div className="flex justify-between text-xs">
                    <span
                      className={`flex items-center gap-1 ${esim.daysRemaining <= 3 ? 'text-red-500 font-bold' : 'text-slate-400'}`}
                    >
                    ...
                    <button
                      onClick={() =>
                        setExpandedChart(isChartOpen ? null : esim.id)
                      }
              ...
              <div className="p-6 text-center">
                <div className="bg-white border-4 border-slate-100 rounded-2xl w-44 h-44 mx-auto mb-5 flex items-center justify-center shadow-inner overflow-hidden">
                  {qrModal.activationCode?.includes('LPA:1$') ? (
                    <img
                      src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrModal.activationCode)}`}
                      alt="QR"
                      className="w-full"
                    />
                  ) : (
                ...
                <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1 mb-4 overflow-hidden">
                  ...
                  <button
                    onClick={() => copyText(qrModal.activationCode)}
            ...
            >
              <div className="bg-gradient-to-r from-violet-500 to-purple-600 p-5 text-center text-white">
                ...
                <p className="text-sm opacity-80">
                  استخدم هذه الإعدادات يدوياً إذا لزم الأمر
                </p>
              </div>
              <div className="p-6 space-y-3">
                ...
                <button
                  onClick={() => setApnModal(null)}
                  className="w-full mt-4 py-3 text-slate-500 hover:text-slate-700 font-medium"
                >
                  إغلاق
                </button>
              </div>
