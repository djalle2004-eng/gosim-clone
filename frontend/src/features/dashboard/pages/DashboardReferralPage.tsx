...
import {
  Copy, Check, Share2, Users, DollarSign, Gift,
  MessageCircle, Link2, ExternalLink,
} from 'lucide-react';

const MOCK_REFERRALS = [
  { name: 'أحمد سعيد', email: 'ahmed@example.com', date: '2026-04-10', orders: 2, earned: 3.0 },
  { name: 'فاطمة زهراء', email: 'fatima@example.com', date: '2026-04-05', orders: 1, earned: 1.5 },
  { name: 'يوسف بوجمعة', email: 'youssef@example.com', date: '2026-03-28', orders: 3, earned: 4.5 },
];
...
export default function DashboardReferralPage() {
  ...
  const stats = [
    { label: 'عدد المُحالين', value: MOCK_REFERRALS.length, icon: Users, color: 'from-cyan-500 to-blue-600', bg: 'bg-cyan-50', text: 'text-cyan-600' },
    { label: 'أرباح معلقة', value: `$${pendingEarned.toFixed(2)}`, icon: DollarSign, color: 'from-amber-400 to-orange-500', bg: 'bg-amber-50', text: 'text-amber-600' },
    { label: 'أرباح محولة', value: `$${paidEarned.toFixed(2)}`, icon: Gift, color: 'from-emerald-500 to-teal-600', bg: 'bg-emerald-50', text: 'text-emerald-600' },
  ];

  return (
    <div dir="rtl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-800 mb-1">برنامج الإحالة</h1>
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-violet-200 text-sm mb-6">احصل على <span className="text-white font-bold">$1.5</span> لكل صديق يُكمل طلبه الأول!</p>
          ...
          <div className="flex bg-white/10 border border-white/20 rounded-2xl overflow-hidden mb-4">
            <div className="flex items-center gap-2 flex-1 px-4 py-3">
              ...
              <span className="text-white/80 text-sm truncate font-mono">{referralLink}</span>
            </div>
            <button onClick={copy}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-5 py-3 text-right text-xs font-bold text-slate-500">الاسم</th>
                <th className="px-5 py-3 text-right text-xs font-bold text-slate-500">تاريخ التسجيل</th>
                <th className="px-5 py-3 text-right text-xs font-bold text-slate-500">عدد الطلبات</th>
                <th className="px-5 py-3 text-right text-xs font-bold text-slate-500">العمولة المكتسبة</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {MOCK_REFERRALS.map((r, i) => (
                <motion.tr key={i}
                  ...
                  <td className="px-5 py-4 font-bold text-emerald-600">${r.earned.toFixed(2)}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
