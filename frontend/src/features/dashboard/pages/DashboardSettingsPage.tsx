        ...
        <div className="lg:w-64 flex-shrink-0">
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            {SECTIONS.map((s) => (
              ...
              >
                <div
                  className={`p-1.5 rounded-lg ${section === s.key ? 'bg-cyan-100' : 'bg-slate-100'}`}
                >
                  <s.icon className="w-4 h-4" />
                </div>
                {s.label}
                <ChevronRight
                  className={`w-4 h-4 mr-auto transition-transform ${section === s.key ? 'rotate-90 text-cyan-500' : 'text-slate-300'}`}
                />
              </button>
            ))}
          </div>
          ...
          >
            ...
            {section === 'profile' && (
              <div className="p-6">
                ...
                <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
                  ...
                  <button className="mr-auto text-sm text-cyan-600 font-medium hover:text-cyan-700">
                    تغيير الصورة
                  </button>
                </div>
                ...
                <button
                  ...
                  className={`mt-5 flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all ${
                    saved
                      ...
                      : 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow hover:shadow-md'
                  }`}
                >
                  {saved ? (
                    <>
            ...
            {section === '2fa' && (
              <div className="p-6">
                ...
                <div
                  className={`flex items-center justify-between p-4 rounded-2xl border mb-6 ${twoFAEnabled ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-xl ${twoFAEnabled ? 'bg-emerald-100' : 'bg-slate-200'}`}
                    >
                      <Shield
                        className={`w-5 h-5 ${twoFAEnabled ? 'text-emerald-600' : 'text-slate-500'}`}
                      />
                    </div>
                    <div>
                      <p className="font-bold text-slate-700 text-sm">
                        {twoFAEnabled ? 'مُفعَّل' : 'غير مُفعَّل'}
                      </p>
                      <p className="text-xs text-slate-400">
                        تحقق ثنائي عبر تطبيق المصادقة
                      </p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    <span
                      className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${twoFAEnabled ? 'translate-x-6 left-0.5' : 'left-0.5'}`}
                    />
                  </button>
                </div>
                {twoFAEnabled && (
              ...
              <div className="p-6">
                ...
                <div className="space-y-3">
                  {MOCK_DEVICES.map((d) => (
                    ...
                    >
                      <Monitor
                        className={`w-8 h-8 ${d.current ? 'text-cyan-500' : 'text-slate-400'}`}
                      />
                      <div className="flex-1">
                  ...
                  {[
                    {
                      ...
                      desc: 'إشعار بريدي عند تغيير حالة الطلب',
                      icon: '📦',
                    },
                    {
                      key: 'email_promos',
                      label: 'العروض والتخفيضات',
                      desc: 'رسائل بريدية حول أحدث العروض',
                      icon: '🎁',
                    },
                    {
                      key: 'push_orders',
                      label: 'إشعارات الطلبات (Push)',
                      desc: 'إشعار فوري عبر المتصفح',
                      icon: '🔔',
                    },
                    {
                      key: 'push_promos',
                      label: 'إشعارات الترويج (Push)',
                      desc: 'عروض حصرية مباشرة على هاتفك',
                      icon: '📢',
                    },
                  ].map((item) => (
                    <div
                    ...
                    >
                      ...
                      >
                        <span
                          className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${notifications[item.key as keyof typeof notifications] ? 'translate-x-5 left-0.5' : 'left-0.5'}`}
                        />
                      </button>
                    </div>
                  ))}
