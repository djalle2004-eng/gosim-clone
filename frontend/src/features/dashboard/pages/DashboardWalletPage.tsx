      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-sm mb-8">
            ≈ {(MOCK_BALANCE_USD * RATES.DZD).toFixed(0)} دج ·{' '}
            {(MOCK_BALANCE_USD * RATES.EUR).toFixed(2)} €
          </p>
        ...
        <div className="divide-y divide-slate-100">
          {MOCK_TRANSACTIONS.map((tx, i) => (
            ...
            >
              ...
              <span
                className={`font-bold text-base ${tx.type === 'credit' ? 'text-emerald-600' : 'text-red-500'}`}
              >
                {tx.type === 'credit' ? '+' : '-'}
                {(tx.amount * RATES[currency]).toFixed(2)} {symbol}
              </span>
            </motion.div>
          ))}
        </div>
