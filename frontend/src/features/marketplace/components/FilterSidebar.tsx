    ...
    <div className="bg-white rounded-3xl border border-slate-100 p-6 sticky top-24">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          >
            المنطقة الجغرافية
            {openSections.regions ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
        ...
        <div className="pt-4 border-t border-slate-100">
          ...
          >
            كمية البيانات
            {openSections.data ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
          {openSections.data && (
            <div>
              <input 
                ...
                onChange={(e) => setFilters({ ...filters, dataAmount: parseInt(e.target.value) })}
              />
              <div className="flex justify-between text-xs text-slate-500 mt-2 font-medium">
                <span>الكل</span>
                <span>{filters.dataAmount === 100 ? 'غير محدود' : `${filters.dataAmount || 0} GB+`}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-slate-100">
          ...
          >
            مدة الصلاحية
            {openSections.validity ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
          {openSections.validity && (
            <div>
              <input 
                ...
                onChange={(e) => setFilters({ ...filters, validity: parseInt(e.target.value) })}
              />
              <div className="flex justify-between text-xs text-slate-500 mt-2 font-medium">
                <span>يوم واحد</span>
                <span>{filters.validity === 365 ? 'سنة كاملة' : `${filters.validity || 1} يوم+`}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-slate-100">
          ...
          >
            سرعة الشبكة
            {openSections.speed ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
          {openSections.speed && (
              ...
              {['4G', '5G'].map(speed => (
                <button
                  ...
                  onClick={() => {
                    ...
                    const updated = current.includes(speed) ? current.filter((s: string) => s !== speed) : [...current, speed];
        ...
        <div className="pt-4 border-t border-slate-100">
          ...
          >
            مزود الخدمة
            {openSections.provider ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
          </button>
