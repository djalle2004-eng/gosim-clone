...
export function PlanCard({ plan, viewMode, onClick }: PlanCardProps) {
  ...
  const getBadge = () => {
    if (plan.isBestSeller) return { text: 'الأكثر مبيعاً', colors: 'from-orange-400 to-pink-500' };
    if (plan.isNew) return { text: 'جديد', colors: 'from-green-400 to-emerald-500' };
    if (plan.isLimited) return { text: 'عرض محدود', colors: 'from-red-400 to-rose-600' };
    return null;
  };
    ...
    return (
      ...
      >
        <div className="md:w-48 h-32 md:h-auto relative overflow-hidden flex-shrink-0">
          ...
          {badge && (
            <div className={`absolute top-3 left-3 bg-gradient-to-r ${badge.colors} text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm`}>
              {badge.text}
            </div>
          )}
        </div>
        ...
        <div className="p-4 flex-1 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <div>
              <h3 className="text-xl font-bold text-slate-900">{plan.countryName}</h3>
          ...
          <div className="flex flex-wrap gap-4 my-3">
             <div className="flex items-center gap-2 text-slate-700">
                ...
                <span className="font-bold">{plan.dataAmount === -1 ? '∞ غير محدود' : `${plan.dataAmount} GB`}</span>
             </div>
             <div className="flex items-center gap-2 text-slate-700">
          ...
          <div className="flex items-center justify-between border-t border-slate-100 pt-3">
            <div>
              <div className="text-xl font-black text-slate-900">${plan.price.toFixed(2)}</div>
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-4 w-full px-4 flex justify-between items-start">
           {badge ? (
            <div className={`bg-gradient-to-r ${badge.colors} text-white text-[11px] font-bold px-3 py-1 rounded-full shadow-md backdrop-blur-md`}>
              {badge.text}
            </div>
          ) : <div></div>}
          <button 
        ...
        <div className="absolute bottom-4 px-4 w-full flex items-end gap-3">
          ...
          <div>
            <h3 className="text-white font-bold text-xl leading-tight">{plan.countryName}</h3>
            <span className="text-white/70 text-sm font-medium">{plan.countryNameEn}</span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-slate-50 p-3 rounded-2xl flex items-center gap-3 border border-slate-100 group-hover:border-cyan-100 transition-colors">
            ...
            <div>
              ...
              <div className="font-bold text-slate-900 text-sm">{plan.dataAmount === -1 ? '∞ غير محدود' : `${plan.dataAmount} GB`}</div>
            </div>
          </div>
          <div className="bg-slate-50 p-3 rounded-2xl flex items-center gap-3 border border-slate-100 group-hover:border-blue-100 transition-colors">
            ...
            <div>
              ...
              <div className="font-bold text-slate-900 text-sm">{plan.validityDays} يوم</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-slate-500 mb-5 px-1">
           ...
           <div className="flex items-center gap-1.5">
             <Globe2 className="w-3 h-3" /> يدعم {plan.supportedNetworksCount || 'عدة'} شبكات
           </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
          <div>
            ...
            <div className="flex items-end gap-2">
              <span className="text-2xl font-black text-slate-900 leading-none">${plan.price.toFixed(2)}</span>
            </div>
            <div className="text-xs text-slate-400 font-medium mt-1">~ {priceDZD} د.ج</div>
          </div>
