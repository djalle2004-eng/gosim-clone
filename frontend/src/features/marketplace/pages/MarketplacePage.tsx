...
export function MarketplacePage() {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      ...
      if (filters.dataAmount) params.append('dataAmount', filters.dataAmount.toString());
      if (filters.validity) params.append('validity', filters.validity.toString());
      if (filters.maxPrice) params.append('maxPrice', filters.maxPrice.toString());
      if (filters.speed?.length) params.append('speed', filters.speed.join(','));
  ...
  const plans = data?.pages.flatMap(page => page.data || page.plans || []) || [];
  
  // Dummy data fallback for UI testing in case API is empty/failing during development
  const dummyPlans = [
    { id: '1', countryName: 'تركيا', countryNameEn: 'Turkey', countryCode: 'TR', flag: '🇹🇷', dataAmount: 10, validityDays: 30, price: 15, isBestSeller: true, networkSpeed: '4G/5G' },
    { id: '2', countryName: 'أوروبا', countryNameEn: 'Europe', countryCode: 'EU', flag: '🇪🇺', dataAmount: 20, validityDays: 30, price: 25, isBestSeller: true, networkSpeed: '5G' },
    { id: '3', countryName: 'عالمي', countryNameEn: 'Global', countryCode: 'GLOBAL', flag: '🌍', dataAmount: -1, validityDays: 15, price: 12, isLimited: true, networkSpeed: '4G' },
    { id: '4', countryName: 'الإمارات', countryNameEn: 'UAE', countryCode: 'AE', flag: '🇦🇪', dataAmount: 3, validityDays: 7, price: 9, isNew: true, networkSpeed: '5G' },
    { id: '5', countryName: 'فرنسا', countryNameEn: 'France', countryCode: 'FR', flag: '🇫🇷', dataAmount: 50, validityDays: 30, price: 18, isBestSeller: true, networkSpeed: '4G/5G' },
    { id: '6', countryName: 'السعودية', countryNameEn: 'Saudi Arabia', countryCode: 'SA', flag: '🇸🇦', dataAmount: 5, validityDays: 15, price: 14, networkSpeed: '5G' },
  ];
  ...
  const activeFiltersCount = Object.values(filters).reduce((acc: number, val: any) => {
    if (Array.isArray(val)) return acc + val.length;
    if (val !== undefined && val !== null && val !== '' && val !== 'popular') return acc + 1;
    return acc;
  }, 0);
    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-2">متجر الباقات</h1>
            <p className="text-slate-500">اختر خطة eSIM المناسبة لوجهتك واستمتع باتصال فوري.</p>
          </div>
          ...
          <div className="flex-1">
            ...
            {activeFiltersCount > 0 && (
               <div className="flex flex-wrap gap-2 mb-6">
                 ...
                 {(filters.regions || []).map((r: string) => (
                   <span key={r} className="bg-cyan-100 text-cyan-800 text-xs px-3 py-1.5 rounded-full flex items-center gap-2">
                     {r}
                     <X className="w-3 h-3 cursor-pointer" onClick={() => setFilters({ ...filters, regions: filters.regions.filter((id: string) => id !== r) })} />
                   </span>
                 ))}
            ...
            {isLoading ? (
              <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div key={i} className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm animate-pulse flex flex-col h-[400px]">
              ...
              <div className="text-center py-20 bg-white rounded-3xl border border-slate-100">
                ...
                <p className="text-slate-500">لم نتمكن من جلب العروض. يرجى المحاولة مرة أخرى لاحقاً.</p>
              </div>
            ) : displayPlans.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl border border-slate-100">
                <div className="text-4xl mb-4">🔍</div>
                <h3 className="text-xl font-bold text-slate-800 mb-2">لا توجد نتائج</h3>
                <p className="text-slate-500">حاول تغيير الفلاتر أو كلمات البحث.</p>
                <button onClick={handleResetFilters} className="mt-4 text-cyan-600 font-bold hover:underline">
                  إعادة تعيين الفلاتر
                </button>
              </div>
            ) : (
        ...
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <div className="relative w-4/5 max-w-sm bg-white h-full overflow-y-auto p-4 animate-in slide-in-from-right">
             <div className="flex justify-between items-center mb-4">
               ...
               <button onClick={() => setShowMobileFilters(false)} className="p-2 bg-slate-100 rounded-full text-slate-600">
                 <X className="w-5 h-5" />
               </button>
             </div>
             <FilterSidebar filters={filters} setFilters={setFilters} onReset={handleResetFilters} />
