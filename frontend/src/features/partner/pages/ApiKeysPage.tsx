...
const ALL_SCOPES = [
  { id: 'plans:read', label: 'Plans — Read', desc: 'List and view eSIM plans' },
  {
    id: 'orders:create',
    label: 'Orders — Create',
    desc: 'Create and manage orders',
  },
  {
    id: 'orders:read',
    label: 'Orders — Read',
    desc: 'View order status and history',
  },
  {
    id: 'esims:read',
    label: 'eSIMs — Read',
    desc: 'View eSIM details and usage',
  },
  {
    id: 'esims:manage',
    label: 'eSIMs — Manage',
    desc: 'Top-up and manage eSIMs',
  },
  {
    id: 'webhooks:write',
    label: 'Webhooks — Write',
    desc: 'Create and delete webhooks',
  },
  {
    id: 'webhooks:read',
    label: 'Webhooks — Read',
...
const MOCK_KEYS = [
  {
    ...
    name: 'Production Key',
    prefix: 'ak_live',
    scopes: ['plans:read', 'orders:create', 'esims:read'],
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-300/70 text-sm mb-3">
                This is the <strong>only time</strong> your full API key will be
                shown. You cannot recover it later.
              </p>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              <th className="text-left text-xs text-slate-500 font-medium px-6 py-4">
                Name
              </th>
              <th className="text-left text-xs text-slate-500 font-medium px-6 py-4">
                Key
          ...
          <tbody className="divide-y divide-white/5">
            {MOCK_KEYS.map((key) => (
              <tr key={key.id} className="hover:bg-white/2 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-1">
                    {key.scopes.slice(0, 2).map((s) => (
                      <span
                        key={s}
                        className="text-xs bg-white/5 text-slate-300 px-2 py-0.5 rounded-full"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <td className="px-6 py-4">
                  <span
                    className={`text-xs font-medium px-2.5 py-1 rounded-full ${key.active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-500/10 text-slate-400'}`}
                  >
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {ALL_SCOPES.map((scope) => (
                    <label
                      key={scope.id}
                      className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedScopes.includes(scope.id) ? 'border-cyan-500/40 bg-cyan-500/5' : 'border-white/5 hover:border-white/10'}`}
                    >
                      <input
                        type="checkbox"
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowCreate(false)}
                  className="flex-1 border border-white/10 text-slate-400 hover:text-white py-3 rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
