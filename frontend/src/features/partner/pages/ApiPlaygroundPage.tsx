...
const ENDPOINTS = [
  {
    id: 'get-plans',
    method: 'GET',
    path: '/v1/partner/plans',
    label: 'List eSIM Plans',
    params: [
  ...
  },
  {
    id: 'create-order',
    method: 'POST',
    path: '/v1/partner/orders',
    label: 'Create Order',
    params: [
      { name: 'planId', type: 'string', default: '' },
          ...
          <div className="bg-[#111120] border border-white/5 rounded-2xl p-4 space-y-3">
            ...
            <div className="space-y-2">
              {ENDPOINTS.map((ep) => (
                ...
                >
                  <span
                    className={`text-xs font-bold px-2 py-0.5 rounded border ${METHOD_COLORS[ep.method]}`}
                  >
                    {ep.method}
                  </span>
        ...
        <div className="bg-[#111120] border border-white/5 rounded-2xl overflow-hidden flex flex-col">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
            <div className="flex items-center gap-3">
              ...
              {response?.status && (
                <span
                  className={`text-xs font-bold px-2.5 py-1 rounded-full ${response.status >= 200 && response.status < 300 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}
                >
                  {response.status}
                </span>
              )}
            ...
            {responseStr && (
              <button
                onClick={() => handleCopy(responseStr)}
