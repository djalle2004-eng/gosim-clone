    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">
          Monitor your API usage limits and manage your subscription.
        </p>
      </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex justify-between text-sm mb-2">
              ...
              <span
                className={`font-medium ${CURRENT_PLAN.dailyUsed / CURRENT_PLAN.dailyLimit > 0.9 ? 'text-red-400' : 'text-white'}`}
              >
                {CURRENT_PLAN.dailyUsed.toLocaleString()} /{' '}
                {CURRENT_PLAN.dailyLimit.toLocaleString()}
              </span>
            </div>
            <ProgressBar
              value={CURRENT_PLAN.dailyUsed}
              max={CURRENT_PLAN.dailyLimit}
              color="cyan"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className="font-medium text-white">
                {CURRENT_PLAN.monthlyOrdersUsed} / {CURRENT_PLAN.monthlyOrders}
              </span>
            </div>
            <ProgressBar
              value={CURRENT_PLAN.monthlyOrdersUsed}
              max={CURRENT_PLAN.monthlyOrders}
              color="violet"
            />
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className="font-medium text-white">
                {CURRENT_PLAN.esimUsed} / {CURRENT_PLAN.esimLimit}
              </span>
            </div>
            <ProgressBar
              value={CURRENT_PLAN.esimUsed}
              max={CURRENT_PLAN.esimLimit}
              color="amber"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          {TIERS.map((tier) => {
            ...
            return (
              <div
                key={tier.name}
                className={`bg-[#111120] border rounded-2xl p-5 ${colorMap[tier.color]} relative`}
              >
                {(tier as any).current && (
                  <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-xs bg-cyan-500 text-slate-900 font-bold px-3 py-0.5 rounded-full">
                    Current
                  </span>
                )}
                ...
                {!(tier as any).current && (
                  <button
                    className={`w-full mt-4 py-2 rounded-xl text-sm font-bold transition-colors ${tier.color === 'violet' ? 'bg-violet-500/20 text-violet-400 hover:bg-violet-500/30' : tier.color === 'amber' ? 'bg-amber-500/20 text-amber-400 hover:bg-amber-500/30' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}
                  >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              <th className="text-left text-xs text-slate-500 px-6 py-3">
                Invoice
              </th>
              <th className="text-left text-xs text-slate-500 px-6 py-3">
                Period
            ...
            {INVOICES.map((inv) => (
              <tr key={inv.id} className="hover:bg-white/2">
                <td className="px-6 py-4">
                  <code className="text-slate-400 text-sm">{inv.id}</code>
                </td>
