...
const ALL_EVENTS = [
  {
    ...
    desc: 'Fired when an order is successfully paid and fulfilled',
  },
  {
    id: 'esim.activated',
    label: 'esim.activated',
    desc: 'Fired when a new eSIM is activated for use',
  },
  {
    id: 'esim.expired',
    label: 'esim.expired',
    desc: 'Fired when an eSIM data pack expires',
  },
  {
    id: 'payment.failed',
    label: 'payment.failed',
    desc: 'Fired when a payment attempt fails',
  },
];
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {MOCK_WEBHOOKS.map((wh) => (
          <div
            key={wh.id}
            className="bg-[#111120] border border-white/5 rounded-2xl overflow-hidden"
          >
            {/* Header */}
            <button
            ...
            >
              <div className="flex items-center gap-4">
                <span
                  className={`w-2 h-2 rounded-full ${wh.active ? 'bg-emerald-400' : 'bg-slate-500'}`}
                />
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {pinging ? (
                    <>
                      <Zap className="w-3 h-3 animate-pulse" /> Sending...
                    </>
                  ) : (
                    <>
            ...
            {expanded === wh.id && (
              <div className="border-t border-white/5">
                <div className="px-5 py-3 text-xs text-slate-500 font-medium uppercase tracking-wider">
                  Delivery Logs
                </div>
                <table className="w-full">
                  <thead>
                    <tr className="bg-white/2">
                      <th className="text-left text-xs text-slate-500 px-5 py-3">
                        Event
                      </th>
                      <th className="text-left text-xs text-slate-500 px-5 py-3">
                        Status
                  ...
                  <tbody className="divide-y divide-white/5">
                    {wh.deliveries.map((d) => (
                      <tr key={d.id} className="hover:bg-white/2">
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`flex items-center gap-1.5 text-xs font-bold ${d.success ? 'text-emerald-400' : 'text-red-400'}`}
                          >
                            {d.success ? (
          ...
          <div className="bg-[#111120] border border-white/10 rounded-3xl p-6 w-full max-w-lg">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {ALL_EVENTS.map((ev) => (
                    <label
                      key={ev.id}
                      className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedEvents.includes(ev.id) ? 'border-cyan-500/40 bg-cyan-500/5' : 'border-white/5 hover:border-white/10'}`}
                    >
                      <input
                        type="checkbox"
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => setShowCreate(false)}
                  className="flex-1 border border-white/10 text-slate-400 py-3 rounded-xl"
                >
                  Cancel
                </button>
                <button
                  onClick={() => setShowCreate(false)}
                  className="flex-1 bg-cyan-500 text-slate-900 font-bold py-3 rounded-xl"
                >
                  Create Endpoint
                </button>
              </div>
