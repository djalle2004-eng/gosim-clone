...
import { CreditCard, Landmark, Phone, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLIC_KEY || 'pk_mock_123');
...
export default function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'stripe' | 'cib' | 'edahabia'>('cib');
  ...
  const { data: plan, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`http://localhost:5000/api/plans/${planId}?currency=DZD`);
  ...
  if (!planId) return <div className="text-white min-h-screen text-center pt-32">يرجى اختيار باقة أولاً</div>;
  if (isLoading) return <div className="text-center w-full min-h-screen pt-32"><div className="w-12 h-12 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin mx-auto"></div></div>;
        ...
        <div className="flex flex-col-reverse lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="lg:w-3/5 w-full space-y-8">
            ...
            <div className="bg-card border border-white/5 rounded-3xl p-8">
              <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-3">
                <span className="w-8 h-8 rounded-full bg-cyan-500/20 text-cyan-400 flex items-center justify-center">1</span>
                المعلومات الشخصية
              </h2>
              
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-400 text-sm mb-2">البريد الإلكتروني للإيصال والـ QR</label>
                  <input 
                    type="email" 
                    value={email}
            ...
            <div className="bg-card border border-white/5 rounded-3xl p-8">
              <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-3">
                <span className="w-8 h-8 rounded-full bg-violet-600/20 text-violet-400 flex items-center justify-center">2</span>
                طريقة الدفع
              </h2>

              <div className="grid grid-cols-3 gap-4 mb-8">
                {[
                  { id: 'cib', label: 'CIB', icon: <Landmark />, color: 'bg-green-500/10 text-green-500 border-green-500/20' },
                  { id: 'edahabia', label: 'الذهبية', icon: <Phone />, color: 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' },
                  { id: 'stripe', label: 'بطاقة دولية', icon: <CreditCard />, color: 'bg-cyan-500/10 text-cyan-500 border-cyan-500/20' }
                ].map(method => (
                  <button
                    key={method.id}
                  ...
                  >
                    <div className={method.color + " p-2 rounded-full"}>{method.icon}</div>
                    <span className="text-white font-medium text-sm md:text-base">{method.label}</span>
                    {paymentMethod === method.id && <CheckCircle2 className="w-4 h-4 text-cyan-400 absolute top-2 right-2" />}
                  </button>
                ))}
              </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {paymentMethod === 'stripe' && clientSecret && (
                    <motion.div key="stripe" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <p className="text-gray-400 mb-6 text-sm">يقوم Stripe بمعالجة المدفوعات الدولية بأسلوب آمن ومعتمد مشفّر.</p>
                  ...
                  {(paymentMethod === 'cib' || paymentMethod === 'edahabia') && (
                    <motion.div key="local" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-8">
                       <Landmark className="w-16 h-16 mx-auto mb-4 text-gray-500 opacity-50" />
                       <h3 className="text-xl font-bold mb-2">الدفع المباشر عبر {paymentMethod === 'cib' ? 'CIB' : 'البطاقة الذهبية'}</h3>
                       <p className="text-gray-400 text-sm mb-8 mx-auto max-w-sm">سيتم تحويلك إلى نافذة الدفع الرسمية الآمنة لـ Satim (البنك الوطني) لإكمال المعاملة.</p>
                       <button 
              ...
              <div className="mt-6 flex items-center justify-center gap-2 text-gray-500 text-xs text-center border-t border-white/5 pt-6">
                <ShieldCheck className="w-4 h-4" /> ضمان حماية البيانات 100%، نحن لا نخزن تفاصيل بطاقتك أبداً.
              </div>
            </div>
          ...
          <div className="lg:w-2/5 w-full">
            <div className="bg-card border border-white/10 rounded-3xl p-8 sticky top-24 shadow-2xl">
              <h3 className="text-white font-bold text-lg border-b border-white/10 pb-4 mb-6">ملخص الطلب</h3>

              <div className="flex items-center gap-4 mb-6">
                 <span className="text-5xl">{plan.country?.flag}</span>
                 <div className="flex-1">
                   <h4 className="text-white font-bold">{plan.name}</h4>
                   <p className="text-gray-400 text-sm tracking-wide">بيانات: {plan.isUnlimited ? "غير محدود" : `${plan.dataAmount} GB`}</p>
                 </div>
                 <div className="text-white font-bold text-xl">{plan.price} د.ج</div>
              </div>
              ...
              <div className="bg-white/5 rounded-xl p-4 flex justify-between items-center mb-6">
                ...
                <span className="text-white font-bold bg-background px-4 py-1.5 rounded-lg border border-white/5">{quantity}</span>
              </div>
              ...
              <div className="flex justify-between items-end mb-2">
                ...
                <div className="text-right">
                  ...
                  <div className="text-gray-500 font-medium text-sm mt-1">ما يعادل ${totalUsd.toFixed(2)} USD</div>
                </div>
              </div>
