...
import { Star, ChevronRight, Zap, Globe, Smartphone, ShieldCheck, Check, Minus, Plus } from 'lucide-react';
...
export default function PlanDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<'specs' | 'activation' | 'reviews'>('specs');
  const [quantity, setQuantity] = useState(1);

  const { data: plan, isLoading } = useQuery({
    queryKey: ['plan', slug],
    queryFn: async () => {
      const res = await fetch(`http://localhost:5000/api/plans/${slug}?currency=DZD`);
  ...
  if (!plan) return <div className="min-h-screen bg-background text-white flex items-center justify-center">لم يتم العثور على الباقة</div>;
    ...
    <div className="min-h-screen flex flex-col bg-background overflow-hidden relative">
      ...
      <main className="flex-grow pt-24 pb-20 container mx-auto px-4 md:px-6">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          <span className="hover:text-white cursor-pointer">الرئيسية</span> <ChevronRight className="w-4 h-4" />
          <span className="hover:text-white cursor-pointer">الباقات</span> <ChevronRight className="w-4 h-4" />
          <span className="hover:text-white cursor-pointer">{plan.country?.nameEn}</span> <ChevronRight className="w-4 h-4" />
          ...
          <div className="lg:w-2/3 w-full">
            ...
            <div className="bg-card border border-white/5 rounded-3xl p-8 mb-8 relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row items-center gap-8 relative z-10 text-center md:text-right">
                <span className="text-8xl drop-shadow-2xl">{plan.country?.flag}</span>
                <div>
                  <div className="flex items-center justify-center md:justify-start gap-3 mb-3">
                    <span className="bg-white/10 text-white text-xs px-3 py-1 rounded-full">{plan.country?.region}</span>
                    {plan.speed === '5G' && <span className="bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 text-xs px-3 py-1 rounded-full font-bold">5G فائق السرعة</span>}
                  </div>
                  <h1 className="text-4xl font-bold text-white mb-2">{plan.name}</h1>
                  <p className="text-xl text-gray-400 mb-4">{plan.country?.nameAr || plan.country?.nameEn} - سعة {plan.isUnlimited ? "غير محدودة" : `${plan.dataAmount} GB`}</p>
                  
                  <div className="flex items-center justify-center md:justify-start gap-2 text-amber-400 cursor-pointer hover:underline" onClick={() => setActiveTab('reviews')}>
                    <Star className="w-5 h-5 fill-current" />
                    <span className="font-bold text-white ml-2 text-lg">4.9</span>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-card border border-white/5 p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                    ...
                    <span className="text-white font-bold">{plan.country?.nameEn}</span>
                  </div>
                  <div className="bg-card border border-white/5 p-4 rounded-2xl flex flex-col items-center justify-center text-center">
                ...
                <div className="bg-card border border-white/5 p-6 rounded-3xl">
                  <h3 className="text-xl font-bold text-white mb-4">وصف الباقة</h3>
                  <p className="text-gray-400 leading-relaxed mb-6">
                    توفر هذه الباقة اتصالاً مباشراً وآمناً على أفضل الشبكات المحلية في {plan.country?.nameEn}. مثالية للسياحة، العمل، أو البقاء على اتصال دائم بالعائلة. لا توجد أي رسوم خفية أو عقود إضافية.
                  </p>
                  
                  <h4 className="font-bold text-white mb-3">الأجهزة المتوافقة:</h4>
                  <p className="text-gray-400 text-sm">
                    تعمل شريحة eSIM حصرياً على الهواتف والأجهزة التي تدعم هذه التقنية وغير المقفلة لشبكة معينة (مثل iPhone 11 فما فوق، Samsung Galaxy S20 فما فوق، وخطوط Google Pixel).
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {activeTab === 'activation' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-card border border-white/5 p-6 md:p-10 rounded-3xl">
                <h3 className="text-xl font-bold text-white mb-8">عملية تثبيت سريعة</h3>
                
                <div className="space-y-8 relative">
                  <div className="absolute top-0 bottom-0 right-[25px] w-0.5 bg-white/10 hidden md:block z-0"></div>

                  {[
                    { title: "اشترِ الباقة", desc: "أكمل عملية الدفع الآمنة للحصول على شريحتك الإلكترونية فوراً." },
                    { title: "امسح الرمز (QR Code)", desc: "سوف يصلك رمز الاستجابة السريعة (QR) على بريدك الإلكتروني وفي لوحة التحكم الخاصة بك." },
                    { title: "الاستمتاع بالإنترنت", desc: "سيتم تفعيل الباقة تلقائياً بمجرد اتصال هاتفك بشبكة الدولة الوجهة." }
                  ].map((step, idx) => (
                ...
                <div className="mt-8 p-4 bg-white/5 border border-white/10 rounded-2xl flex flex-col md:flex-row items-center gap-6 justify-between filter grayscale blur-[2px] opacity-60">
                   <div className="w-32 h-32 bg-white rounded-xl flex items-center justify-center text-black font-bold">QR CODE</div>
                   <p className="text-center md:text-right">سيظهر الرمز الخاص بك هنا فور اكتمال الدفع بنجاح.</p>
                </div>
              </motion.div>
            )}

            {activeTab === 'reviews' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 bg-card rounded-3xl border border-white/5">
                <Star className="w-16 h-16 text-white/10 mx-auto mb-4" />
                <h3 className="text-xl font-bold text-white mb-2">تقييمات العملاء</h3>
                <p className="text-gray-400">ستتوفر مراجعات هذه الباقة قريباً من قبل مستخدمينا.</p>
              </motion.div>
            )}
          ...
          <div className="lg:w-1/3 w-full lg:sticky lg:top-24">
            <div className="bg-card border border-white/10 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group">
              ...
              <div className="mb-8 border-b border-white/10 pb-8 text-center text-white">
                <span className="block text-gray-500 text-sm font-medium mb-3">السعر الإجمالي</span>
                ...
                <div className="text-gray-400 font-medium">${(plan.priceUsd * quantity).toFixed(2)} USD</div>
              </div>
              ...
              <div className="mb-6 bg-background rounded-2xl border border-white/5 p-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-bold text-white">{plan.validity} أيام</span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-gray-400">حجم البيانات:</span>
                  <span className="font-bold text-white">{plan.isUnlimited ? "غير محدود" : `${plan.dataAmount} GB`}</span>
                </div>
                {plan.retailPrice && (
                  <div className="flex justify-between items-center">
                    <span className="text-gray-400">سعر التوفير:</span>
                    <span className="font-bold text-emerald-400 line-through">د.ج {(plan.retailPrice * quantity).toLocaleString()}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between mb-8 bg-background border border-white/5 p-2 rounded-2xl">
                ...
                <div className="text-xl font-bold flex-1 text-center text-white">
                  {quantity} 
                  <span className="text-sm text-gray-500 font-normal mr-2">شرائح</span>
                </div>
                <button 
