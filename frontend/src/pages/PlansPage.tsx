...
export default function PlansPage() {
  ...
  const activeFiltersCount = Array.from(searchParams.keys()).filter(k => k !== 'page' && k !== 'sortBy').length;
      ...
      <main className="flex-grow pt-24 pb-20 relative">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold text-white mb-6">اكتشف باقات الإنترنت</h1>
            <SearchBar />
          </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-card border border-white/5 p-4 rounded-2xl">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="text-white font-bold">
                     {isLoading ? 'جاري البحث...' : `تم العثور على ${data?.pagination?.total || 0} باقة`}
                  </span>
              ...
              {isLoading ? (
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {[...Array(6)].map((_, i) => (
                    <div key={i} className="h-[450px] bg-white/5 rounded-3xl animate-pulse"></div>
                  ))}
                </div>
                ...
                <div className="bg-card border border-white/5 rounded-3xl p-12 text-center py-32">
                  ...
                  <p className="text-gray-400 mb-8 max-w-md mx-auto">
                    لم نعثر على أي باقات إنترنت تتطابق مع فلاتر البحث الحالية. جرب تغيير بعض الفلاتر لرؤية المزيد من النتائج.
                  </p>
                  <button 
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-6 pt-2">
                ...
                <button onClick={() => setIsMobileFilterOpen(false)} className="p-2 bg-white/5 rounded-full text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <FilterSidebar />
              ...
              <div className="sticky bottom-0 bg-background/90 backdrop-blur pt-4 pb-2 mt-8 border-t border-white/5">
                 <button onClick={() => setIsMobileFilterOpen(false)} className="w-full bg-gradient-to-r from-violet-600 to-cyan-500 text-white py-4 rounded-xl font-bold">
