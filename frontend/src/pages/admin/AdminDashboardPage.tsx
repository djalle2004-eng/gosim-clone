...
import {
  DollarSign, ShoppingBag, Users, Activity, TrendingUp, CreditCard,
...
const sparkData = (seed: number) =>
  Array.from({ length: 12 }, (_, i) => ({
    v: Math.max(10, Math.round(seed + Math.sin(i + seed) * seed * 0.3 + Math.random() * seed * 0.2)),
  }));
...
const topPlans = [
  { name: 'Europe 10GB 30 Days', orders: 842, revenue: '$12,360', growth: '+18%' },
...
const recentTransactions = [
  { id: '#TXN-9821', user: 'Ahmed Bensalem', plan: 'Europe 10GB', amount: '$14.99', status: 'success', time: '2m ago' },
  { id: '#TXN-9820', user: 'Sarah Dupont', plan: 'Asia 3GB', amount: '$9.99', status: 'success', time: '5m ago' },
  { id: '#TXN-9819', user: 'Mohammed Al-Rashid', plan: 'Global 5GB', amount: '$12.99', status: 'failed', time: '8m ago' },
  { id: '#TXN-9818', user: 'Yasmine Cherif', plan: 'USA Unlimited', amount: '$10.00', status: 'success', time: '12m ago' },
  { id: '#TXN-9817', user: 'Carlos Ruiz', plan: 'MENA 2GB', amount: '$8.99', status: 'pending', time: '15m ago' },
];
      ...
      <div className="bg-slate-900 border border-white/10 rounded-xl p-3 shadow-2xl text-sm">
        ...
        {payload.map((p: any) => (
          <p key={p.name} style={{ color: p.color }} className="font-bold">
            {p.name === 'revenue' ? 'Current' : 'Previous'}: ${p.value.toLocaleString()}
          </p>
        ))}
      </div>
    );
  ...
  return (
    <div className="bg-white border border-slate-200/60 rounded-2xl p-5 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`flex items-center gap-0.5 text-sm font-bold ${kpi.isPositive ? 'text-emerald-500' : 'text-red-500'}`}>
          {kpi.isPositive ? <ArrowUpRight className="w-4 h-4" /> : <ArrowDownRight className="w-4 h-4" />}
          {kpi.change}
        </span>
      </div>
      <div className="text-2xl font-black text-slate-800 mb-0.5">{kpi.value}</div>
      <div className="text-xs text-slate-400 mb-3">{kpi.label}</div>
      {/* Sparkline */}
      <div className="h-10" dir="ltr">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={kpi.spark}>
            <Line type="monotone" dataKey="v" stroke={kpi.sparkColor} strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    ...
    <div className="max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Real-time performance overview</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleRefresh} className={`p-2 text-slate-400 hover:text-slate-700 border border-slate-200 rounded-xl transition-all ${refreshing ? 'animate-spin' : ''}`}>
      ...
      <div className="bg-white border border-slate-200/60 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-400">Current period vs previous period</p>
          </div>
        ...
        <div className="h-72" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={displayData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="previous" stroke="#8b5cf6" strokeWidth={1.5} strokeDasharray="4 4"
                fill="url(#colorPrev)" />
              <Area type="monotone" dataKey="revenue" stroke="#06b6d4" strokeWidth={2.5}
                fill="url(#colorRev)" dot={false} activeDot={{ r: 5, fill: '#06b6d4' }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-700 truncate">{plan.name}</p>
                    <p className="text-xs text-slate-400">{plan.orders} orders</p>
                  </div>
                </div>
                <div className="text-right shrink-0 ml-2">
                  <p className="text-sm font-bold text-slate-800">{plan.revenue}</p>
                  <p className={`text-xs font-medium ${plan.growth.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}`}>
                    {plan.growth}
                  </p>
                </div>
              </div>
              ...
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-800">{c.revenue}</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full"
                      style={{ width: `${(c.orders / topCountries[0].orders) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-1 bg-white border border-slate-200/60 rounded-2xl p-5">
          ...
          <div className="space-y-3">
            {recentTransactions.map((tx, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 truncate">{tx.user}</p>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100">
            <p className="text-xs text-slate-400 mb-2">Today's hourly transactions</p>
            <div className="h-16" dir="ltr">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={Array.from({ length: 12 }, (_, i) => ({ h: i * 2, v: Math.round(Math.random() * 20 + 5) }))}>
