...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      {/* Sidebar fixed */}
      <aside className="w-72 border-l border-slate-200/50 bg-white flex flex-col h-screen sticky top-0 z-50">
        <div className="h-20 px-6 flex items-center border-b border-slate-200/50">
        ...
        <div className="p-4 border-t border-slate-200/50">
          <button
            ...
            className="flex items-center gap-3 w-full px-4 py-3 text-slate-500 hover:text-red-400 hover:bg-red-400/10 rounded-xl font-medium transition-colors"
          >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* Top Header */}
        <header className="h-20 border-b border-slate-200/50 bg-white/80 backdrop-blur-md sticky top-0 z-40 px-8 flex items-center justify-between">
          <div className="flex-1 flex items-center">
            <input
              type="text"
              placeholder="ابحث عن مستخدم، طلب، أو باقة..."
              className="w-96 bg-slate-50 border border-slate-200/50 rounded-full px-6 py-2.5 outline-none text-slate-800 focus:border-cyan-500/50 transition-colors text-sm"
            />
          </div>

          <div className="flex items-center gap-6">
            <button className="relative w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors">
