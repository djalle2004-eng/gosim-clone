...
function OrderDetailModal({
  ...
  const updateStatus = useMutation({
    mutationFn: () =>
      api.patch(`/admin/orders/${order.id}/status`, { status: newStatus }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-orders'] });
      onClose();
    },
    onError: () => {
      /* already handled via mock */ onClose();
    },
  });

  const processRefund = useMutation({
    mutationFn: () =>
      api.post(`/admin/orders/${order.id}/refund`, { reason: refundReason }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-orders'] });
      onClose();
    },
          ...
          {!showRefund ? (
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4">
              <p className="text-sm font-bold text-red-700 mb-2">
                Confirm Refund of ${order.totalAmount}
              </p>
              <textarea
                value={refundReason}
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-500"
                >
                  Cancel
                </button>
              </div>
...
export default function AdminOrdersPage() {
  ...
  const filtered = useMemo(() => {
    return orders.filter((o) => {
      const matchSearch =
        search === '' ||
        `${o.id} ${o.userName} ${o.userEmail} ${o.planName}`
          .toLowerCase()
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {[
                  ...
                  <th
                    key={h}
                    className="py-3 px-4 text-left font-semibold text-slate-500"
                  >
                    {h}
                  </th>
                ))}
              ...
              {paginated.map((order) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4">
                      <span
                        className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium ${cfg.color}`}
                      >
