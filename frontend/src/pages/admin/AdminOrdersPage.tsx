  ...
  const data = useMemo(
    () => [
      ...
      },
      {
        id: 'ORD-9879',
        user: 'amine@test.com',
        plan: 'الولايات المتحدة (غير محدود)',
        amount: 5400,
        payment: 'Stripe',
        status: 'COMPLETED',
    ...
    () => [
      ...
      },
      {
        ...
        cell: (info: any) => {
          ...
          return (
            <span
              className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${s === 'COMPLETED' ? 'bg-emerald-500/20 text-emerald-400' : s === 'PROCESSING' ? 'bg-orange-500/20 text-orange-400' : 'bg-red-500/20 text-red-400'}`}
            >
              {s}
            </span>
          );
