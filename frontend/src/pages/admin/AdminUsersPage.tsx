...
import {
  Search, Download, MoreVertical, Ban, Mail,
  ChevronLeft, ChevronRight, Eye, UserCog, CheckSquare
} from 'lucide-react';
...
const MOCK_USERS: User[] = Array.from({ length: 50 }, (_, i) => ({
  id: `u${i}`,
  firstName: ['Ahmed', 'Sara', 'Mohammed', 'Yasmine', 'Carlos', 'Liu', 'Fatima', 'James'][i % 8],
  lastName: ['Bensalem', 'Dupont', 'Al-Rashid', 'Cherif', 'Ruiz', 'Wei', 'Zoubi', 'Brown'][i % 8],
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-slate-800 to-slate-900 p-6 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${user.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                {user.isActive ? 'Active' : 'Suspended'}
              </span>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-white p-2 rounded-xl hover:bg-white/10 transition-colors">✕</button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Change Role */}
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Change Role</label>
    ...
    <div className="max-w-[1600px] mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">{filtered.length.toLocaleString()} users total</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="py-3 px-4 text-left font-semibold text-slate-500">User</th>
                <th className="py-3 px-4 text-left font-semibold text-slate-500">Role</th>
                <th className="py-3 px-4 text-left font-semibold text-slate-500">Orders</th>
                <th className="py-3 px-4 text-left font-semibold text-slate-500">Country</th>
                <th className="py-3 px-4 text-left font-semibold text-slate-500">Joined</th>
                <th className="py-3 px-4 text-left font-semibold text-slate-500">Status</th>
                <th className="py-3 px-4 text-right font-semibold text-slate-500">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {paginated.map(user => (
                <tr key={user.id} className={`hover:bg-slate-50/50 transition-colors ${selectedIds.has(user.id) ? 'bg-cyan-50/50' : ''}`}>
                  ...
                  <td className="py-3.5 px-4">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${ROLE_BADGE[user.role] || ROLE_BADGE.USER}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="py-3.5 px-4 font-medium text-slate-700">{user._count?.orders ?? '—'}</td>
                  <td className="py-3.5 px-4 text-slate-500">{user.country || '—'}</td>
                  <td className="py-3.5 px-4 text-slate-500 text-xs">
                    {new Date(user.createdAt).toLocaleDateString('en-GB')}
                  </td>
                  <td className="py-3.5 px-4">
                    <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium ${user.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
