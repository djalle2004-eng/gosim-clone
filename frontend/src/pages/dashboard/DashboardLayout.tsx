        ...
        <div className="bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200 rounded-2xl p-4 mb-6">
          <div className="flex items-center gap-3">
            <img
              src={`https://i.pravatar.cc/40?u=${user.id}`}
              alt=""
              className="w-10 h-10 rounded-xl border border-cyan-300/40"
            />
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur border-t border-slate-200 flex z-50 px-2 pb-safe">
        {navItems.slice(0, 5).map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                'flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 text-[10px] font-medium relative transition-colors',
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <div className="absolute top-0 inset-x-2 h-0.5 bg-cyan-500 rounded-b-full" />
                )}
