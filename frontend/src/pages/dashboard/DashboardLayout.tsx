...
import { Smartphone, ReceiptText, UserCircle, Bell, LifeBuoy, LogOut, ShieldCheck } from 'lucide-react';
...
export default function DashboardLayout() {
  ...
  const navItems = [
    { label: 'eSIM الخاصة بي', path: '/dashboard/esims', icon: <Smartphone className="w-5 h-5" /> },
    { label: 'طلباتي', path: '/dashboard/orders', icon: <ReceiptText className="w-5 h-5" /> },
    { label: 'الملف الشخصي', path: '/dashboard/profile', icon: <UserCircle className="w-5 h-5" /> },
    { label: 'الإشعارات', path: '/dashboard/notifications', icon: <Bell className="w-5 h-5" /> },
    { label: 'الدعم', path: '/dashboard/support', icon: <LifeBuoy className="w-5 h-5" /> },
  ];
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      <header className="md:hidden bg-card border-b border-white/5 py-4 px-4 sticky top-0 z-50 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-violet-600 to-cyan-500 flex items-center justify-center text-white font-bold shadow-lg">G</div>
          <span className="font-bold tracking-tight text-white">Go<span className="text-cyan-400">SIM</span></span>
        </Link>
        <img src={user.avatar} alt="Avatar" className="w-8 h-8 rounded-full border border-cyan-500/30" />
      </header>
      ...
      <aside className="hidden md:flex w-72 flex-col bg-card border-l border-white/5 h-screen sticky top-0 p-6 z-40">
        <Link to="/" className="flex items-center gap-2 group mb-10">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-violet-600 to-cyan-500 flex items-center justify-center text-white font-bold text-xl shadow-lg">G</div>
          <span className="text-2xl font-bold tracking-tight text-white">Go<span className="text-cyan-400">SIM</span></span>
        </Link>
        ...
        <div className="bg-background border border-white/5 rounded-2xl p-4 mb-8 flex items-center gap-4 relative overflow-hidden group">
          <img src={user.avatar} alt="Avatar" className="w-12 h-12 rounded-full border-2 border-cyan-500/30" />
        ...
        <nav className="flex-1 space-y-2">
          {navItems.map((item) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <div className={cn("transition-transform group-hover:scale-110", isActive && "text-cyan-400")}>
                    {item.icon}
                  </div>
                  {item.label}
                  {item.label === 'الإشعارات' && (
                    <span className="mr-auto w-5 h-5 bg-violet-600 text-white flex items-center justify-center rounded-full text-[10px] font-bold">2</span>
                  )}
                </>
              )}
            </NavLink>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-white/5 flex px-2 pb-safe z-50">
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && <div className="absolute top-0 inset-x-4 h-1 bg-cyan-400 rounded-b-full"></div>}
