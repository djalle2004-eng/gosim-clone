...
import { QrCode, Copy, RefreshCw, MoreVertical, AlertTriangle } from 'lucide-react';
...
export default function MyESimsPage() {
  ...
  const esims = orders?.flatMap((order: any) => 
    order.orderItems.flatMap((item: any) => 
      item.assignedESims.map((esim: any) => ({
        ...
        daysRemaining: esim.expiresAt ? Math.max(0, Math.ceil((new Date(esim.expiresAt).getTime() - new Date().getTime()) / (1000 * 3600 * 24))) : item.plan.validity,
        iccid: esim.iccid,
  ...
  return (
    <>
      <div className="mb-8">
        ...
        <p className="text-gray-400">قم بإدارة وتتبع استهلاك باقات الإنترنت الحالية والسابقة.</p>
      </div>

      {esims.length === 0 ? (
        <div className="bg-card border border-white/5 rounded-3xl p-12 text-center py-32">
           <h3 className="text-2xl font-bold text-white mb-2">لا توجد باقات مفعلة</h3>
           <p className="text-gray-400 mb-8 max-w-md mx-auto">
             يبدو أنك لم تقم بشراء أي شريحة eSIM بعد. يمكنك استكشاف باقاتنا المميزة.
           </p>
        </div>
      ) : (
          ...
          {esims.map((esim: any, idx: number) => (
            <motion.div 
              ...
              className={`bg-card border rounded-3xl p-6 relative overflow-hidden flex flex-col ${
                esim.status === 'ACTIVE' ? 'border-emerald-500/50 shadow-lg shadow-emerald-500/5' : 
                esim.status === 'INACTIVE' ? 'border-cyan-500/30' : 'border-white/5 opacity-75'
              }`}
            >
              {/* Status Head */}
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">{esim.planName}</h3>
                    <p className="text-gray-400 text-sm">ICCID: {esim.iccid.slice(-6)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                    esim.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/20' :
                    esim.status === 'INACTIVE' ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/20' :
                  ...
                  <button className="text-gray-500 hover:text-white transition-colors p-1"><MoreVertical className="w-5 h-5"/></button>
                </div>
              </div>
              ...
              {esim.status !== 'INACTIVE' && (
                <div className="bg-background rounded-2xl border border-white/5 p-4 mb-6 relative overflow-hidden">
                  <div className="flex justify-between text-sm mb-3">
                    ...
                    <span className="text-white font-bold">{esim.usedData} GB / {esim.isUnlimited ? "∞" : `${esim.totalData} GB`}</span>
                  </div>
                  ...
                  <div className="w-full bg-white/10 rounded-full h-3 mb-4 backdrop-blur-sm overflow-hidden flex">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: esim.isUnlimited ? '100%' : `${(parseFloat(esim.usedData) / parseFloat(esim.totalData)) * 100}%` }}
                      transition={{ duration: 1, ease: 'easeOut' }}
                      className={`h-full bg-gradient-to-r rounded-full ${
                        !esim.isUnlimited && (parseFloat(esim.usedData) / parseFloat(esim.totalData)) > 0.8 ? 'from-orange-500 to-red-500' : 'from-emerald-400 to-cyan-500'
                      }`}
                    />
                  </div>

                  <div className="flex justify-between items-center text-xs">
                    <div className={`flex items-center gap-1.5 ${esim.daysRemaining <= 3 ? 'text-red-400 font-bold' : 'text-gray-400'}`}>
                      <AlertTriangle className="w-4 h-4" /> 
                      {esim.daysRemaining} أيام متبقية
                    </div>
                    {esim.status === 'ACTIVE' && (
                      <div className="flex items-center gap-1 text-emerald-400">
                        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span> متصل
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="bg-gradient-to-r from-violet-600 to-cyan-500 p-6 text-center">
                <h3 className="text-white font-bold text-xl mb-1">تثبيت شريحة eSIM</h3>
                <p className="text-white/80 text-sm">امسح الكود عبر إعدادات الشبكة الخلوية في هاتفك</p>
              </div>
              ...
              <div className="p-8">
                 ...
                 <div className="bg-white p-4 rounded-2xl w-48 h-48 mx-auto mb-6 flex justify-center items-center shadow-inner overflow-hidden">
                   {selectedEsim.activationCode?.includes('LPA:1$') ? (
                     <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(selectedEsim.activationCode)}`} alt="QR Code" className="w-full h-full object-contain" />
                   ) : (
                     <img src={selectedEsim.activationCode} alt="Base64 QR" className="w-full h-full object-contain" />
                   )}
                 </div>

                 {/* Manual Code */}
                 <div className="mb-6">
                   <label className="block text-gray-500 text-xs mb-2 text-center">أو أدخل الرمز يدوياً</label>
                   <div className="flex bg-background border border-white/10 rounded-xl p-1 overflow-hidden">
                     <code className="flex-1 px-4 py-3 text-xs text-cyan-400 font-mono truncate self-center">{selectedEsim.activationCode?.slice(0,25)}...</code>
                     <button 
                 ...
                 <button onClick={() => setQrModalOpen(false)} className="w-full py-4 text-gray-400 hover:text-white font-medium transition-colors">إغلاق وتخطي</button>
              </div>
            </motion.div>
          </div>
        )}
